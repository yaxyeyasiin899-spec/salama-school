...
export default function Login({ onLogin, students, teachers }: { onLogin: (user: UserType) => void, students: Student[], teachers: Teacher[] }) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username === 'yaxye' && password === '123') {
      onLogin({ id: 'admin-1', username: 'yaxye', role: 'Admin', name: 'Maamule Yaxye' });
      return;
    }

    // Check Students
    const student = students.find(s => s.username.toLowerCase().trim() === username.toLowerCase().trim() && s.password === password);
    if (student) {
      onLogin({ id: `student-${student.id}`, username: student.username, role: 'Arday', name: student.name });
      return;
    }

    // Check Teachers
    const teacher = teachers.find(t => t.username.toLowerCase().trim() === username.toLowerCase().trim() && t.password === password);
    if (teacher) {
      onLogin({ id: `teacher-${teacher.id}`, username: teacher.username, role: 'Macalin', name: teacher.name });
      return;
    }
  ...
  return (
    <div className="flex flex-col md:flex-row h-full min-h-screen bg-blue-50">
      <div className="md:w-1/2 bg-blue-600 rounded-b-[3rem] md:rounded-b-none md:rounded-r-[3rem] p-8 flex flex-col items-center justify-center text-white shadow-lg h-64 md:h-auto shrink-0 relative overflow-hidden">
        ...
        <h1 className="text-3xl md:text-5xl font-bold tracking-wider z-10">SCHOOL SALAMA</h1>
        <p className="text-blue-100 mt-2 font-medium z-10 md:text-xl">Maamulka Dugsiga</p>
      </div>
      ...
      <div className="flex-1 px-8 md:px-24 pt-10 pb-6 flex flex-col justify-center bg-white">
        <div className="max-w-md w-full mx-auto flex flex-col h-full md:justify-center">
          <div className="mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-center text-gray-800">Soo Dhawoow!</h2>
            <p className="text-center text-gray-500 mt-2">Geli ID-gaaga iyo Password-ka</p>
          </div>
          ...
          <div className="mt-8 bg-gray-50 p-4 rounded-xl border border-gray-100 text-sm">
            <p className="font-bold text-gray-700 mb-2">Tijaabo (Demo Accounts):</p>
            <ul className="text-gray-600 space-y-1">
              <li>Admin: <span className="font-mono bg-gray-200 px-1 rounded">yaxye</span> / <span className="font-mono bg-gray-200 px-1 rounded">123</span></li>
              <li>Arday: <span className="font-mono bg-gray-200 px-1 rounded">Axmed Cali Faarax</span> / <span className="font-mono bg-gray-200 px-1 rounded">123</span></li>
              <li>Macalin: <span className="font-mono bg-gray-200 px-1 rounded">Maxamed Faarax Cali</span> / <span className="font-mono bg-gray-200 px-1 rounded">123</span></li>
            </ul>
          </div>
